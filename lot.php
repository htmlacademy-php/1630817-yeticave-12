...
if(isset($_GET['lot_id']) && !empty($_GET['lot_id']) && is_numeric($_GET['lot_id'])){
    ...
    if(!empty($lot)){
        ...
        print (include_template('layout.php', ['categories' => $categories,'user_name' => 'Mansur', 'is_auth' => $is_auth,'title' => 'Ошибка 404', 'main_content' => include_template('404.php',['categories' => $categories])]));
    }
}
else{
    print (include_template('layout.php', ['categories' => $categories,'user_name' => 'Mansur', 'is_auth' => $is_auth,'title' => 'Ошибка 404', 'main_content' =>include_template('404.php',['categories' => $categories])]));
